...
export async function onRequest(context) {
  ...
  // CORS preflight (optional; images can be embedded cross-origin without this).
  ...
  try {
    row = await env.DB
      .prepare("SELECT thumb_png AS png FROM vf_maps WHERE id = ?")
      .bind(mapId)
      .first();
  } catch (e) {
    // Likely DB not migrated yet (no such column) or other DB error.
    return new Response("thumb_png_not_available", { status: 404, headers: { "Access-Control-Allow-Origin": "*" } });
  }

  const png = row?.png;
  if (!png)
    return new Response("thumb_not_found", { status: 404, headers: { "Access-Control-Allow-Origin": "*" } });

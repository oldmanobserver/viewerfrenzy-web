...
export async function onRequest(context) {
  ...
  let row = null;
  try {
    row = await db.prepare("SELECT image_png AS png FROM vf_maps WHERE id = ?").bind(mapId).first();
  } catch (e) {
    // Likely DB not migrated yet (no such column) or other DB error.
    return new Response("image_png_not_available", {
      status: 404,
  ...
  const png = row?.png;
  if (!png) {
    return new Response("image_not_found", {
      status: 404,
      headers: { "Access-Control-Allow-Origin": "*" },
    });
  }
